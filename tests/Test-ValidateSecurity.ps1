<#
.SYNOPSIS
    Prueba Validate-Security.ps1 contra los fixtures de sistema simulado.
.DESCRIPTION
    Ejecuta scripts/validation/Validate-Security.ps1 con -SystemInfoPath apuntando
    a cada JSON de tests/fixtures/security/, compara el Status esperado (campo
...
$validatorPath = Join-Path -Path $projectRoot -ChildPath "scripts\validation\Validate-Security.ps1"
$manifestPath = Join-Path -Path $projectRoot -ChildPath "manifests\production-pro\8.3\security.json"
...
$fixturesRoot = Join-Path -Path $testsRoot -ChildPath "fixtures\security"
...
if (-not (Test-Path -Path $fixturesRoot -PathType Container)) {
    Write-Host "No se encontro la carpeta de fixtures de seguridad en: $fixturesRoot" -ForegroundColor Red
    exit 1
}
...
Write-Host ""
Write-Host "Test-ValidateSecurity" -ForegroundColor Cyan
...
foreach ($fixture in $fixtures) {
    ...
    # Mostrar detalles de checks individuales en modo depuracion / diagnostico
...
$noAdminFixture = Join-Path -Path $fixturesRoot -ChildPath "security-no-admin.json"

if ((Test-Path -Path $hardwareEnforcing -PathType Leaf) -and (Test-Path -Path $noAdminFixture -PathType Leaf)) {
    $totalCases++
    $expectedEnforcing = "Fail"

    try {
        $enforcingResult = & $validatorPath -SystemInfoPath $noAdminFixture -ManifestPath $manifestPath -AssessmentPath $assessmentPath -HardwareManifestPath $hardwareEnforcing
        $actualEnforcing = [string]$enforcingResult.Status

        if ($actualEnforcing -eq $expectedEnforcing) {
            Write-Host "$passIcon security-no-admin.json (enforcing): esperado=$expectedEnforcing obtenido=$actualEnforcing" -ForegroundColor Green
        }
        else {
            Write-Host "$failIcon security-no-admin.json (enforcing): esperado=$expectedEnforcing obtenido=$actualEnforcing" -ForegroundColor Red
            $failedCases++
        }
        ...
        Write-Host "$failIcon security-no-admin.json (enforcing): error al ejecutar el validador: $($_.Exception.Message)" -ForegroundColor Red
        $failedCases++
    }
    Write-Host ""
}
else {
    Write-Host "$failIcon No se encontro el manifiesto enforcing o el fixture security-no-admin para el caso extra." -ForegroundColor Red
    $totalCases++
    $failedCases++
    Write-Host ""
}
